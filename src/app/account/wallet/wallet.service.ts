...
export class WalletService {
  ...
  constructor(
    ...
    private httpService: HttpService) {
      ...
      let accounts = localStorage.getItem(bitumConfig.name + '-wallets');
  ...
  saveToStorage() {
    localStorage.setItem(bitumConfig.name + '-wallets', JSON.stringify(this.accounts))
  }

  saveToHomeStorage(accounts) {
    localStorage.setItem(bitumConfig.name + '-home', JSON.stringify(accounts))
  }

  getFromHomeStorage() {
    let home = localStorage.getItem(bitumConfig.name + '-home');
    if (home) {
  ...
  getFromWalletStorage() {
    let home = localStorage.getItem(bitumConfig.name + '-wallets');
    if (home) {
      return JSON.parse(home);
    }
