...
export class MessengerService {
  ...
  constructor() {
  	let history = localStorage.getItem(bitumConfig.name + '-messenger-history');
  	if (history) {
  		try {
  ...
  getConversationMessages(id) {
  	let key = bitumConfig.name + '-messenger-messages-' + id;
  ...
  saveHistoryToStorage() {
  	localStorage.setItem(bitumConfig.name + '-messenger-history', JSON.stringify(this.history));
  	this.onHistoryUpdated.emit(this.history);
  }

  saveMessagesToStorage(id, messages) {
    let _messages = _.orderBy(messages, ['createdAt'], ['asc']);
    localStorage.setItem(bitumConfig.name + '-messenger-messages-' + id, JSON.stringify(_messages));
    this.onMessageUpdated.emit(messages);
  }

  saveMessage(walletAddress, message) {
    let key = bitumConfig.name + '-messenger-wallet-' + walletAddress;
